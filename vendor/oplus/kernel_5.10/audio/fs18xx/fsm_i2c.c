...
static void fsm_spin_work(struct work_struct *work)
{
    fsm_config_t *cfg = fsm_get_config();
    fsm_stereo_rotation(cfg->next_angle);
}
            ...
            const struct i2c_device_id *id)
{
    ...
    if(fsm_dev->id == 0) {
        ...
        cfg->spin_wq = create_singlethread_workqueue("spin-wq");
        INIT_DELAYED_WORK(&cfg->spin_work, fsm_spin_work);
    }
...
static int fsm_i2c_remove(struct i2c_client *i2c)
{
    ...
    if (cfg->spin_wq) {
        cancel_delayed_work_sync(&cfg->spin_work);
        destroy_workqueue(cfg->spin_wq);
        cfg->spin_wq = NULL;
    }
