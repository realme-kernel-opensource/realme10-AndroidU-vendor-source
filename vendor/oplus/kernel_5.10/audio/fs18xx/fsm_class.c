				...
				struct class_attribute *attr, char *buf)
{
	...
	for (dev = 0; dev < result->ndev; dev++) {
		...
		size += scnprintf(buf + size, PAGE_SIZE, "[%02X,%d,%d]",
				info->addr, info->re25*1000/1024, info->errcode);
	}
...
static ssize_t fsm_delay_show(struct class *class,
				struct class_attribute *attr, char *buf)
{
	return scnprintf(buf, PAGE_SIZE, "delay ms: %d\n", g_delay_ms);
}

static ssize_t fsm_delay_store(struct class *class,
				struct class_attribute *attr, const char *buf, size_t len)
{
	g_delay_ms = simple_strtoul(buf, NULL, 0);
	pr_info("delay ms:%d", g_delay_ms);
	return len;
}

static ssize_t fsm_swap_show(struct class *class,
				struct class_attribute *attr, char *buf)
{
    fsm_config_t *cfg = fsm_get_config();
...
static ssize_t fsm_swap_store(struct class *class,
				struct class_attribute *attr, const char *buf, size_t len)
{
	int angle = simple_strtoul(buf, NULL, 0);

    pr_info("angle: %d", angle);
	fsm_delay_ms(g_delay_ms);
...
static struct class_attribute g_fsm_class_attrs[] = {
	...
	__ATTR(reg, S_IRUGO|S_IWUSR, fsm_reg_show, fsm_reg_store),
        __ATTR(rotation, S_IRUGO|S_IWUSR, fsm_swap_show, fsm_swap_store),
	__ATTR(delay, S_IRUGO|S_IWUSR, fsm_delay_show, fsm_delay_store),
...
static CLASS_ATTR_RW(fsm_swap);
static CLASS_ATTR_RW(fsm_delay);
...
static struct attribute *fsm_class_attrs[] = {
	...
	&class_attr_fsm_reg.attr,
        &class_attr_fsm_swap.attr,
	&class_attr_fsm_delay.attr,
