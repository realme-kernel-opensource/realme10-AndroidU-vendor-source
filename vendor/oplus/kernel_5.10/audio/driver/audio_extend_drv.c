...
static const char *extend_speaker_fs_prop[CODEC_PROP_MAX] = {
	[CODEC_I2S_ID] = "oplus,speaker-i2s-id",
	[CODEC_I2S_ID_IN] = "oplus,speaker-i2s-id-in",
	[CODEC_NAME] = "oplus,speaker-codec-name-fs",
	[CODEC_DAI_NAME] = "oplus,speaker-codec-dai-name-fs",
	[CODEC_VENDOR] = "oplus,speaker-vendor-fs",
};
...
struct audio_extend_data {
	...
	struct codec_prop_info *spk_pa_fs_info;
...
void extend_codec_i2s_be_dailinks(struct snd_soc_dai_link *dailink, size_t size)
{
	...
	if (g_smartpa_type == MTK_SPK_FOURSEMI_FS18XX && g_extend_pdata->use_extern_dailink_spk &&
	    g_extend_pdata->spk_pa_fs_info) {
		extend_codec_be_dailink(g_extend_pdata->spk_pa_fs_info, dailink, size);
	} else if (g_extend_pdata->use_extern_dailink_spk && g_extend_pdata->spk_pa_info) {
...
{
	g_smartpa_type = smartpa_type;
}
EXPORT_SYMBOL(extend_set_smartpa_type);
...
static int audio_extend_probe(struct platform_device *pdev)
{
	...
	if (of_property_read_bool(pdev->dev.of_node, "oplus,is_multi_smartpa")) {
		g_extend_pdata->spk_pa_fs_info =  devm_kzalloc(&pdev->dev, sizeof(struct codec_prop_info), GFP_KERNEL);
		if (g_extend_pdata->spk_pa_fs_info) {
			ret = extend_codec_prop_parse(&pdev->dev, extend_speaker_fs_prop, g_extend_pdata->spk_pa_fs_info);
			if (ret == 0) {
				g_extend_pdata->use_extern_dailink_spk = true;
			} else {
				g_extend_pdata->use_extern_dailink_spk = false;
